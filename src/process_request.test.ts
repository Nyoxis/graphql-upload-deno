...
import { Upload, UploadError, UploadErrorCode } from "./index.ts";
import { processRequest } from "./process_request.ts";
import { createMultipartMessage, attachStreamEndDetection } from "../test/utils.ts";
...
Deno.test("processRequest loads simple file and parses variables", async () => {
  ...
  // Assert on query
  assertEquals(result.query, "mutation ($file: Upload!) { upload(file: $file) }");
...
Deno.test("processRequest loads batched request and returns batched result", async () => {
  ...
  assertEquals(result[0].query, "mutation ($file: Upload!) { upload(file: $file) }");
  ...
  assertExists(result[1].variables, "should't be variables for the second operation");
});
...
Deno.test("processRequest silently ignores extraneous files", async () => {
  ...
  const multipartBody = createMultipartMessage(boundary, {
    ...
    f2: {
      ...
      mediaType: "text/plain",
      content: new TextEncoder().encode("This file is extraneous and should be ignored")
    }
  });
